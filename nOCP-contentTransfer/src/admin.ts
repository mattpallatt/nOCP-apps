...
// ported near-verbatim from nocp-widget's/nocp-frontify's admin.ts — see
// nocp-frontify's CLAUDE.md, "The settings page is a reusable pattern".
// This app has no webhook receiver, so there's no second "recent webhooks"
// section bolted onto the bootstrap function at the bottom. Fully generic
// against SETTINGS_SCHEMA — needed zero changes to render this app's 24
// fields across 4 sections instead of nocp-frontify's 11 across 3.
...
function fieldInputHtml(field: SettingField, value: FieldValue | undefined): string {
  ...
  if (field.type === "toggle") {
    ...
    // (the ClientSecret/ContentGraphSecret fields) gets the same monospace
