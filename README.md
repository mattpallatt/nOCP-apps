...
Four apps live here, side by side:
...
- **[`nOCP-contentTransfer/`](nOCP-contentTransfer/)** — the largest and most involved app here: pushes content-item trees and content-model definitions between Optimizely CMS SaaS environments. Worth a look specifically for `jobStore.ts` — a checkpoint/lock pattern that folds a long-running, multi-step background job into the same short HTTP action the frontend already polls every 1.5s, which is how this app gets job-scheduling-shaped behavior with no AWS infra beyond the one Lambda every other app here already needs (no Step Functions, no self-invoking Lambda, no SQS).
...
All four deploy the same way: one AWS Lambda behind a public Function URL, no API Gateway, no server to manage, free within Lambda's (and, for `nOCP-widget/`/`nOCP-contentTransfer/`, DynamoDB's) always-free tier. Each has its own README/`CLAUDE.md` with a quick-start.
...
All four apps here implement 1–4 and 6 in full; `nOCP-widget/` and `nOCP-contentTransfer/` implement 5 and 7 completely (including the icon upload); `nOCP-categoryExplorer/` implements 5, 7 (without an icon uploaded by default), and 8; `nOCP-starter/` implements 5 and a `hasIcon: false` version of 7 (it has nowhere to persist an uploaded icon — see its own README for exactly what would need to be added back).
