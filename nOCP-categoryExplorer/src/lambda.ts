...
// flag. Passed as a header (never a query param).
...
// returns the masked admin view (toAdminView hides secret-masked fields —
// just singleKey here), POST returns the masked view of whatever the
...
// Unauthenticated shell — per the nOCP app spec §2, a bare page load can't
// attach a custom header, so the real gate happens client-side: admin.ts
// prompts for the token, then does all actual reads/writes as fetch()
// calls carrying X-NOCP-Admin-Token. The shell itself carries no data.
function renderAdminShell(): APIGatewayProxyResultV2 {
  const page =
    '<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Category Explorer — Admin</title>' +
...
export const handler = async (
  ...
  if (path === '/nocp/meta' && method === 'GET') {
    ...
    return {
      ...
      // displayMode hardcoded 'sidebar' — matches the original OCP
      // extension's own ui_extensions.sidebar declaration, not a runtime
      // choice.
  ...
  // Action router — POST /category-explorer/api {action, params}. Gated by
  ...
  if (path === '/category-explorer/api' && method === 'POST') {
    const settings = await getSettings();
