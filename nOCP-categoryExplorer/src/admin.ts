...
// Shared core (token gate, schema-driven field rendering/save/regenerate)
// ported near-verbatim from nocp-widget's admin.ts, via nocp-frontify's copy
// — see that app's CLAUDE.md, "The settings page is a reusable pattern".
// This app has no webhook receiver, so unlike nocp-widget's copy there's no
// second "recent webhooks" section bolted onto the bootstrap function at
// the bottom.
...
function fieldInputHtml(field: SettingField, value: FieldValue | undefined): string {
  ...
  if (field.type === "toggle") {
    ...
    inputHtml = `<input type="number" id="field-${field.key}" data-key="${field.key}" value="${escapeAttr(String(value ?? ""))}">`;
    ...
    // (singleKey — a pasted-in third-party credential) gets the same
    // monospace styling but shows whatever masked/plain string the server
    // chose to send back — this function doesn't decide that, it just
    // displays it.
...
async function loadAdminPage(): Promise<void> {
  ...
  if (!settingsRes.ok) {
    root.innerHTML = `<p class="help">Failed to load settings: HTTP ${settingsRes.status}</p>`;
    return;
  }

  const settings = (await settingsRes.json()) as SettingsValues;

  root.innerHTML = `<div id="settings-section"></div>`;
